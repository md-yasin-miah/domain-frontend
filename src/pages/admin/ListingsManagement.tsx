...
export default function ListingsManagement() {
  ...
  const filters = useMemo(
    () => ({
      ...
      listing_type_id:
        listingTypeId === "all" ? undefined : Number(listingTypeId),
      ...(viewMode === "my" && statusFilter !== "all"
        ? { status: statusFilter as MarketplaceListing["status"] }
        : {}),
    }),
    [page, size, searchTerm, listingTypeId, viewMode, statusFilter],
  );

  const { data: allListingsData, isLoading: isLoadingAll } =
    useGetMarketplaceListingsQuery(filters, {
      skip: viewMode !== "all",
    });
  const { data: myListingsData, isLoading: isLoadingMy } =
    useGetMyMarketListingQuery(filters, {
      skip: viewMode !== "my",
    });
  ...
  const items: MarketplaceListing[] = Array.isArray(data)
    ? data
    : (data?.items ?? []);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast({
        ...
        description:
          t("admin.listings.delete_success_desc") ??
  ...
  const handleFeature = async (listing: MarketplaceListing) => {
    try {
      ...
      const message =
        error instanceof Error
          ? error.message
          : t("admin.listings.feature_error");
      toast({
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-2">
          ...
          <span
            className={cn(
              "capitalize rounded-md border px-2 py-0.5 text-xs font-medium",
              getStatusBadgeVariant(row.status) === "default"
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm">
            {t("admin.listings.description") ??
              "View and manage marketplace listings. Change status, feature, or delete."}
          </p>
        </div>
        <Button
          asChild
          className="border text-primary border-border bg-card hover:bg-muted/50"
        >
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-3">
        ...
        {viewMode === "my" && (
          ...
          >
            <SelectTrigger className="w-[140px] border-border">
              <SelectValue
                placeholder={t("admin.listings.filter_status") ?? "Status"}
              />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="all">
                {t("admin.listings.status_all") ?? "All"}
              </SelectItem>
        ...
        >
          <SelectTrigger className="w-[160px] border-border">
            <SelectValue
              placeholder={t("admin.listings.filter_type") ?? "Type"}
            />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="all">
              {t("admin.listings.type_all") ?? "All types"}
            </SelectItem>
        ...
        <div className="relative flex-1">
          ...
          <Input
            placeholder={
              t("admin.listings.search_placeholder") ??
      ...
      <Card className="border border-border">
        <CardContent className="pt-6">
          <DataTableWithPagination
            ...
            isLoading={isLoading}
            emptyMessage={
              t("admin.listings.no_listings") ?? "No listings found."
            }
            emptyIcon={<List className="h-12 w-12 text-muted-foreground" />}
            getRowId={(row) => String(row.id)}
            enableSorting={true}
            onRowClick={(row) =>
              navigate(`/admin/listings-management/view/${row.id}`)
            }
            renderActions={(row) => (
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="border-border">
                  <DropdownMenuLabel>
                    {t("admin.listings.table.actions") ?? "Actions"}
                  </DropdownMenuLabel>
                  <DropdownMenuSeparator />
                  <DropdownMenuItem
                    onClick={() =>
                  ...
                  <DropdownMenuItem
                    onClick={() =>
                      navigate(`/admin/listings-management/edit/${row.id}`)
                    }
                  >
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              {listingToDelete && (
                <div className="mt-2 p-2 rounded border border-border bg-muted/30 font-medium">
