...
import { Search, Loader2, TrendingUp, BarChart3, Globe, Sparkles, Calendar, Hash, Link2, Server } from "lucide-react";
...
const AppValuationsIndex = () => {
  ...
  const [trigger, { data, isFetching, isSuccess, error }] = useLazyGetAutoValuationQuery();
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-background via-background to-muted/30">
      ...
      <section className="relative overflow-hidden px-4 pt-8 pb-12 sm:px-6 lg:px-8">
        ...
        <div className="relative mx-auto max-w-3xl text-center">
          ...
          <p className="mt-3 text-muted-foreground text-lg">
            Enter a domain to get an instant estimate, market rank, and comparable sales.
          </p>
          ...
          {error && (
            <Alert variant="destructive" className="mt-6 text-left max-w-md mx-auto sm:mx-0">
      ...
      {/* Loading skeleton */}
      {isFetching && (
        <div className="px-4 pb-12 sm:px-6 lg:px-8 max-w-5xl mx-auto space-y-6">
...
function ValuationResults({ data }: { data: AutoValuationResponse }) {
  ...
  const hasDomainInfo = data.domain_info && Object.values(data.domain_info).some((v) => v != null && (Array.isArray(v) ? v.length > 0 : true));
  const hasValuationData = data.valuation_data && Object.keys(data.valuation_data).length > 0;
  ...
  return (
    <div className="px-4 pb-16 sm:px-6 lg:px-8 max-w-5xl mx-auto space-y-8">
      ...
      <Card className="overflow-hidden border-primary/20 bg-gradient-to-br from-card to-primary/5">
        <CardHeader className="pb-2">
          ...
          <p className="text-muted-foreground">
            Range: {formatCurrency(data.min_estimate)} – {formatCurrency(data.max_estimate)} {currency}
          </p>
        </CardHeader>
        <CardContent className="flex flex-wrap items-center gap-3">
          <Badge variant="secondary" className="text-sm" title={`Name: ${data.domain_name} · Extension: ${data.domain_extension}`}>
            {data.domain}
          </Badge>
          <span className="text-sm text-muted-foreground">
            Name: <strong className="text-foreground">{data.domain_name}</strong> · Ext: <strong className="text-foreground">.{data.domain_extension}</strong>
          </span>
          {data.market_rank && (
            <Badge
              variant={data.market_rank.tier === "premium" ? "default" : "outline"}
              className="capitalize"
              title={`Rank ${data.market_rank.rank} of ${data.market_rank.total_count}`}
            >
              {data.market_rank.tier.replace("-", " ")} · Top {data.market_rank.percentile}% (rank {data.market_rank.rank}/{data.market_rank.total_count})
            </Badge>
          )}
          <span className="text-sm text-muted-foreground">
            Based on {data.comparable_sales_count} comparable sale{data.comparable_sales_count !== 1 ? "s" : ""}
          </span>
        </CardContent>
      </Card>
        ...
        {hasAnyMetric && (
          <Card>
            ...
            <CardContent className="space-y-2 text-sm">
              {data.domain_length_score != null && (
                <p>
                  <span className="text-muted-foreground">Length score:</span> {data.domain_length_score}/100
                </p>
              )}
              {data.domain_age_years != null && (
                <p>
                  <span className="text-muted-foreground">Age:</span> {data.domain_age_years} year{data.domain_age_years !== 1 ? "s" : ""}
                </p>
              )}
              {data.domain_authority_score != null && (
                <p>
                  <span className="text-muted-foreground">Domain authority:</span> {data.domain_authority_score}
                </p>
              )}
              {data.backlinks_count != null && (
                <p>
                  <span className="text-muted-foreground">Backlinks:</span> {formatNumber(data.backlinks_count)}
                </p>
              )}
              {data.monthly_traffic != null && (
                <p>
                  <span className="text-muted-foreground">Monthly traffic:</span> {formatNumber(data.monthly_traffic)}
                </p>
              )}
              {data.monthly_revenue != null && (
                <p>
                  <span className="text-muted-foreground">Monthly revenue:</span> {formatCurrency(Number(data.monthly_revenue))} {currency}
                </p>
              )}
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-2 text-sm">
              <p>
                ...
                {formatCurrency(data.market_trend.average_sale_price)} {data.market_trend.currency}
              </p>
              <p>
                <span className="text-muted-foreground">Median:</span>{" "}
                {formatCurrency(data.market_trend.median_sale_price)} {data.market_trend.currency}
              </p>
              {data.market_trend.total_sales_count != null && (
                <p>
                  <span className="text-muted-foreground">Total sales (period):</span> {data.market_trend.total_sales_count}
                </p>
              )}
              ...
              {data.market_trend.price_change_percentage != null && (
                <p>
                  ...
                  >
                    {data.market_trend.price_change_percentage >= 0 ? "+" : ""}
                    {Number(data.market_trend.price_change_percentage).toFixed(1)}%
                  </span>
                </p>
              )}
            </CardContent>
          ...
          <Card>
            ...
            <CardContent className="space-y-2 text-sm">
              {data.domain_info.domain_age_years != null && (
                <p>
                  <span className="text-muted-foreground">Age (from WHOIS):</span> {data.domain_info.domain_age_years} year
                  {data.domain_info.domain_age_years !== 1 ? "s" : ""}
                </p>
              )}
              {data.domain_info.creation_date != null && (
                <p>
                  <span className="text-muted-foreground">Created:</span> {moment(data.domain_info.creation_date).format("lll")}
                </p>
              )}
              {data.domain_info.expiration_date != null && (
                <p>
                  <span className="text-muted-foreground">Expires:</span> {moment(data.domain_info.expiration_date).format("lll")}
                </p>
              )}
              {data.domain_info.updated_date != null && (
                <p>
                  <span className="text-muted-foreground">Updated:</span> {moment(data.domain_info.updated_date).format("lll")}
                </p>
              )}
              {data.domain_info.registrar != null && (
                <p>
                  <span className="text-muted-foreground">Registrar:</span> {data.domain_info.registrar}
                </p>
              )}
              {data.domain_info.page_links_count != null && (
                <p>
                  <span className="text-muted-foreground">Page links:</span> {data.domain_info.page_links_count}
                </p>
              )}
              {data.domain_info.name_servers != null && data.domain_info.name_servers.length > 0 && (
                <div>
      ...
      {hasValuationData && data.valuation_data && (
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardDescription>Additional data used or produced by the valuation model</CardDescription>
          </CardHeader>
          <CardContent>
      ...
      {data.recent_comparable_sales && data.recent_comparable_sales.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle>Recent comparable sales</CardTitle>
            <CardDescription>Similar domains in the same extension (domain_name, domain_extension, sale_price, sale_date, currency)</CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b text-left text-muted-foreground">
                    <th className="pb-3 pr-4 font-medium">Domain (name · ext)</th>
                  ...
                  {data.recent_comparable_sales.map((sale, i) => (
                    <tr key={i} className="border-b last:border-0">
                      ...
                      <td className="py-3 pr-4 text-right">
                        {formatCurrency(Number(sale.sale_price))} {sale.currency}
                      </td>
                      <td className="py-3 text-muted-foreground">
                        {sale.sale_date ? moment(sale.sale_date).format("lll") : "—"}
                      </td>
                    </tr>
                  ))}
                </tbody>
