...
const ClientDomainsPage = () => {
  ...
  const baseFilteredDomains =
    domainsData?.items?.filter((domain) =>
  ...
  const getFilteredDomainsByTab = (tab: string) => {
    ...
    switch (tab) {
      case "active":
        return baseFilteredDomains.filter(
  ...
  const DomainCard = ({
    ...
    domain: NonNullable<typeof domainsData>["items"][number];
  }) => {
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <CardDescription className="flex items-center gap-4 mt-1">
                  <span>
                    {t("domains.registered_by")} {domain.seller.username}
                  </span>
                  <Badge
                    variant={
                      domain.listing_type.name === "Premium"
                        ? "default"
                        : "secondary"
                    }
                  >
            ...
            <div className="flex items-center gap-2">
              {daysUntilExpiration <= 30 && daysUntilExpiration > 0 && (
                ...
                >
                  ...
                  {t("domains.expires_in")} {daysUntilExpiration}{" "}
                  {t("domains.days")}
                </Badge>
              )}
          ...
          <div className="grid md:grid-cols-3 gap-6">
            ...
            <div className="space-y-3">
              <h4 className="font-semibold text-sm text-muted-foreground">
                {t("domains.sections.domain_info")}
              </h4>
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <Badge
                    variant={domain?.is_auto_renew ? "default" : "secondary"}
                    className="text-xs"
                  >
                    {domain?.is_auto_renew
                      ? t("domains.auto_renewal.enabled")
                      : t("domains.auto_renewal.disabled")}
                  </Badge>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">
                    {timeFormat(domain.updated_at, "lll")}
                  </span>
                </div>
              </div>
            ...
            <div className="space-y-3">
              <h4 className="font-semibold text-sm text-muted-foreground">
                {t("domains.sections.quick_actions")}
              </h4>
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-muted-foreground mt-2">{t("domains.subtitle")}</p>
        </div>
      ...
      <Tabs defaultValue="all" className="w-full">
        <TabsList className="grid w-full grid-cols-4">
          <TabsTrigger value="all">
            {t("domains.tabs.all")} ({domainsData?.pagination.total || 0})
          </TabsTrigger>
          <TabsTrigger value="active">
            {t("domains.tabs.active")} ({activeCount})
          </TabsTrigger>
          <TabsTrigger value="expiring">
            {t("domains.tabs.expiring")} ({expiringCount})
          </TabsTrigger>
          <TabsTrigger value="expired">
            {t("domains.tabs.expired")} ({expiredCount})
          </TabsTrigger>
        </TabsList>
          ...
          {getFilteredDomainsByTab("all").length > 0 ? (
            ...
            <div className="text-center py-12">
              ...
              <h3 className="text-xl font-semibold mb-2">
                {t("domains.empty_states.active.title")}
              </h3>
              <p className="text-muted-foreground">
                {searchTerm
                  ? t("common.no_results") || "No domains match your search"
          ...
          {getFilteredDomainsByTab("active").length > 0 ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-muted-foreground">
                {searchTerm
                  ? t("common.no_results") ||
                    "No active domains match your search"
                  : t("domains.empty_states.active.description")}
              </p>
            </div>
          )}
        ...
        <TabsContent value="expiring" className="mt-6">
          {getFilteredDomainsByTab("expiring").length > 0 ? (
            ...
            <div className="text-center py-12">
              ...
              <h3 className="text-xl font-semibold mb-2">
                {t("domains.empty_states.expiring.title")}
              </h3>
              <p className="text-muted-foreground">
                {searchTerm
                  ? t("common.no_results") ||
                    "No expiring domains match your search"
        ...
        <TabsContent value="expired" className="mt-6">
          {getFilteredDomainsByTab("expired").length > 0 ? (
            ...
            <div className="text-center py-12">
              ...
              <h3 className="text-xl font-semibold mb-2">
                {t("domains.empty_states.expired.title")}
              </h3>
              <p className="text-muted-foreground">
                {searchTerm
                  ? t("common.no_results") ||
                    "No expired domains match your search"
