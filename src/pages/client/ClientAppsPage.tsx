...
const ClientAppsPage = () => {
  ...
  const AppCard = ({
    ...
    app: NonNullable<typeof appsData>["items"][number];
  }) => {
    return (
      <Card className="hover:shadow-lg transition-shadow">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <CardDescription className="flex items-center gap-4 mt-1">
                  <span>
                    {t("apps.registered_by")} {app.seller.username}
                  </span>
                  <Badge
                    variant={
                      app.listing_type.name === "Premium"
                        ? "default"
                        : "secondary"
                    }
                  >
        ...
        <CardContent>
          <div className="grid md:grid-cols-3 gap-6">
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">
                    {app.website_technology || "---"}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>{t("apps.labels.last_update")}</span>
                  <span className="font-medium">
                    {timeFormat(app.updated_at, "lll")}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3">
              <h4 className="font-semibold text-sm text-muted-foreground">
                {t("apps.sections.stats")}
              </h4>
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <span>{t("apps.labels.downloads")}</span>
                  <span className="font-medium">
                    {formatNumber(app.website_traffic_monthly)}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>{t("apps.labels.active_users")}</span>
                  <span className="font-medium">
                    {formatNumber(app.view_count)}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>{t("apps.labels.revenue_monthly")}</span>
                  <span className="font-medium">
                    {formatCurrency(app.website_revenue_monthly)}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>{t("apps.labels.profit_monthly")}</span>
                  <span className="font-medium">
                    {formatCurrency(app.website_profit_monthly)}
                  </span>
                </div>
              </div>
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-muted-foreground mt-2">{t("apps.subtitle")}</p>
        </div>
      ...
      <Tabs defaultValue="all" className="w-full">
        <TabsList className="grid w-full grid-cols-2">
          <TabsTrigger value="all">
            {t("apps.tabs.all")} ({appsData?.pagination.total || 0})
          </TabsTrigger>
          <TabsTrigger value="active">
            {t("apps.tabs.active")} ({activeCount})
          </TabsTrigger>
        </TabsList>
          ...
          {getFilteredAppsByTab("all").length > 0 ? (
            ...
            <div className="text-center py-12">
              ...
              <h3 className="text-xl font-semibold mb-2">
                {t("apps.empty_states.all.title")}
              </h3>
              <p className="text-muted-foreground">
                {searchTerm
                  ? t("common.no_results") || "No apps match your search"
                  : t("apps.empty_states.all.description")}
              </p>
            </div>
          )}
        </TabsContent>
        ...
        <TabsContent value="active" className="mt-6">
          {getFilteredAppsByTab("active").length > 0 ? (
            ...
            <div className="text-center py-12">
              <Globe className="w-16 h-16 text-muted-foreground mx-auto mb-4" />
              <h3 className="text-xl font-semibold mb-2">
                {t("apps.empty_states.active.title")}
              </h3>
              <p className="text-muted-foreground">
                {searchTerm
                  ? t("common.no_results") || "No active apps match your search"
                  : t("apps.empty_states.active.description")}
              </p>
            </div>
          )}
        </TabsContent>
