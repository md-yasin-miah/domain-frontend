...
import { FlagIcon, Globe, Loader2 } from 'lucide-react';
import { useGetLanguagesQuery } from '@/store/api/i18nApi';

const FALLBACK_LANGUAGES = [
  { code: 'en', name: 'English', native_name: 'English', flag: '🇺🇸', is_default: true },
  { code: 'es', name: 'Spanish', native_name: 'Español', flag: '🇪🇸', is_default: false },
];

const LanguageSwitcher = () => {
  const { i18n } = useTranslation();
  const { data, isLoading } = useGetLanguagesQuery();
  const languages = data?.languages?.length ? data.languages : FALLBACK_LANGUAGES;
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="ghost" size="sm" className="gap-2 bg-gray-100">
          ...
          <span className="uppercase">{currentLang?.code ?? currentCode}</span>
        </Button>
      </DropdownMenuTrigger>
